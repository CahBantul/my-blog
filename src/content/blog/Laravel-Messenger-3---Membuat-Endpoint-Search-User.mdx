...
tags:
  - laravel
title: 'Laravel Messenger #3 - Membuat Endpoint Search User'
---
...
Perintah ini akan membuat file /app/Http/Controllers/UserController.php.Selanjutnya, Anda perlu membuat metode search di dalam UserController untuk menangani permintaan pencarian pengguna. Dalam metode ini, Anda akan menulis logika untuk melakukan pencarian pengguna berdasarkan kriteria yang diberikan.
...
## Langkah 4: Menggunakan Endpoint Pencarian

Sekarang, Anda dapat menggunakan endpoint pencarian yang telah Anda buat dalam aplikasi Anda. Ini bisa dilakukan dengan mengirimkan permintaan GET ke /users/search dengan parameter query yang berisi kata kunci pencarian.

```shell
curl http://laravel-messenger.test/api/users/search?query=john
```

Aplikasi Anda akan mengembalikan daftar pengguna yang nama mereka cocok dengan kriteria pencarian 'john'.
